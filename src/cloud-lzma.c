...
unsigned char *lzmaCompCollectData(unsigned char *srcBuf, int srcLen, int *destLen, int type)
{
	int dstLen = 0;
	int error = 0;
	unsigned char *dstBuf = NULL;
	
	if (NULL == srcBuf || NULL == destLen || 0 >= srcLen)
	{
		MSG_PRINT("srcBuf or destLen is NULL.\n");
		return NULL;
	}
	...
	{
		dstLen = getDstLenFromBuf(srcBuf);
		if (0 >= dstLen)
		{
	...
	if (NULL == dstBuf)
	{
		MSG_PRINT("No enough memory for dest buf!\n");
		return NULL;
	}
...
#if 0
unsigned char *lzmaUncompCollectData(unsigned char *srcBuf, int srcLen, int *destLen)
{
	int ret = -1;
	int dstLen = 0;
	int error = 0;
...
int lzmaCompData(char *srcFileName, char *dstFileName)
{
	...
	dstBuf = lzmaCompCollectData(srcBuf, srcLen, &dstLen, CLOUD_LZMA_COMPRESS);
	if (0 >= dstLen || NULL == dstBuf)
	{
		FREE_P(srcBuf);
		FREE_P(dstBuf);
...
int lzmaUncompData(char *srcFileName, char *dstFileName)
{
	...
	dstBuf = lzmaCompCollectData(srcBuf, srcLen, &dstLen, CLOUD_LZMA_UNCOMPRESS);
	if (0 >= dstLen || NULL == dstBuf)
	{
		FREE_P(srcBuf);
		FREE_P(dstBuf);
